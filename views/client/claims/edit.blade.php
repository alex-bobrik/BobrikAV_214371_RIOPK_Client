...
@section('title', 'Редактирование убытка')
@section('page-title', 'Редактирование страхового убытка')
...
<div class="card shadow-sm">
    <div class="card-header">
        <h5 class="mb-0">Редактирование убытка №{{ $claim->id }}</h5>
    </div>
        ...
        <form id="editClaimForm">
            ...
            <div class="row g-3">
                    ...
                    <select class="form-select select2" id="contract_id" name="contract_id" required>
                        ...
                        @foreach($contracts as $contract)
                            <option value="{{ $contract->id }}" 
                                @if($contract->id === $claim->contract_id) selected @endif>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="amount" name="amount" value="{{ $claim->amount }}" required>
                    <div class="invalid-feedback" id="amountError"></div>
                </div>

                <div class="col-12">
                    <label for="description" class="form-label">Описание *</label>
                    <textarea class="form-control" id="description" name="description" rows="4" required>{{ $claim->description }}</textarea>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-success">Убыток обновлен</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Закрыть"></button>
            </div>
            <div class="modal-body">
                <p>Изменения успешно сохранены.</p>
                <p>Номер убытка: <strong>{{ $claim->id }}</strong></p>
            </div>
            <div class="modal-footer">
                <a href="{{ route('client.claims.index') }}" class="btn btn-secondary">К списку убытков</a>
                <a href="{{ route('client.claims.show', $claim->id) }}" class="btn btn-primary">Просмотреть убыток</a>
            </div>
        </div>
    </div>
</div>
